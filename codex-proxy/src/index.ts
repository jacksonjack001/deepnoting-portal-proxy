...
function printStartupHint(message: string): void {
  if (/config\/default\.yaml|config\/fingerprint\.yaml|yaml/i.test(message)) {
    console.error("[Init] Make sure config/default.yaml and config/fingerprint.yaml exist and are valid YAML.");
    return;
  }

  if (/native transport addon|codex-tls/i.test(message)) {
    console.error("[Init] Install Rust, then run 'cd native && npm install && npm run build'.");
  }
}
...
export async function startServer(options?: StartOptions): Promise<ServerHandle> {
  ...
  // Initialize TLS transport (Rust native addon)

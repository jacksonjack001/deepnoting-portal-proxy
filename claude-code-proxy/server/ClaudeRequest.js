...
class ClaudeRequest {
  ...
  constructor(req = null) {
    ...
    this.currentAuthAccountId = null;
    this.currentAuthSource = null;
  ...
  isDashboardProbe() {
    const flag = this.requestHeaders['x-dashboard-probe'];
  ...
  async getFallbackAuthContext() {
    if (!FALLBACK_TO_CLAUDE_CODE) {
      throw new Error('Claude Code credential fallback is disabled.');
    }

    return {
      token: await this.loadFromClaudeCodeCredentials(),
      accountId: null,
      source: 'claude_code'
    };
  }
  ...
  async getAuthToken() {
    ...
    if (ClaudeRequest.cachedToken) {
      if (this.currentAuthSource !== 'manual') {
        this.currentAuthAccountId = OAuthManager.getActiveAccountId();
        this.currentAuthSource = this.currentAuthSource || 'oauth';
      }
      return ClaudeRequest.cachedToken;
    }

    const authContext = await this.loadOrRefreshToken();
    if (authContext.accountId) {
      ClaudeRequest.cachedToken = authContext.token;
    }
    this.currentAuthAccountId = authContext.accountId;
    this.currentAuthSource = authContext.source;
  ...
  async loadOrRefreshToken() {
    try {
      ...
      if (OAuthManager.isAuthenticated()) {
        ...
        const accountId = OAuthManager.getActiveAccountId();
  ...
  getRequestBetaHeaders() {
    const betaHeader = this.requestHeaders['anthropic-beta'];
  ...
  getBetaHeaders(body = null) {
    const betas = new Set([
      ...this.DEFAULT_BETA_HEADERS,
      ...this.getRequestBetaHeaders()
    ]);

    if (body && body.context_management) {
      betas.add('context-management-2025-06-27');

      const edits = Array.isArray(body.context_management.edits)
        ? body.context_management.edits
        : [];

      if (edits.some(edit => edit && edit.type === 'compact_20260112')) {
  ...
  getHeaders(token, body = null) {
    ...
    const betaHeaders = this.getBetaHeaders(body);
  ...
  async sendRequest(token, body, presetName = null) {
    const processedBody = this.processRequestBody(this.cloneBody(body), presetName);
    const headers = this.getHeaders(token, processedBody);
  ...
  async makeRequest(body, presetName = null) {
    ...
    return this.sendRequest(token, body, presetName);
  }

  async executeWithAuthFallback(body, presetName = null) {
    let response = await this.makeRequest(body, presetName);
    if (response.statusCode !== 401) {
      return response;
    }

    Logger.info('Got 401 from Claude API, retrying authentication flow');
    ClaudeRequest.cachedToken = null;

    try {
      const refreshedAuth = await this.loadOrRefreshToken();
      if (refreshedAuth.accountId) {
        ClaudeRequest.cachedToken = refreshedAuth.token;
      }
      this.currentAuthAccountId = refreshedAuth.accountId;
      this.currentAuthSource = refreshedAuth.source;

      response = await this.sendRequest(refreshedAuth.token, body, presetName);
      if (response.statusCode !== 401) {
        return response;
      }
    } catch (error) {
      Logger.info(`Primary token reload failed: ${error.message}`);
    }

    if (FALLBACK_TO_CLAUDE_CODE && this.currentAuthSource !== 'claude_code') {
      try {
        const fallbackAuth = await this.getFallbackAuthContext();
        this.currentAuthAccountId = fallbackAuth.accountId;
        this.currentAuthSource = fallbackAuth.source;
        response = await this.sendRequest(fallbackAuth.token, body, presetName);
      } catch (error) {
        Logger.info(`Claude Code fallback failed: ${error.message}`);
      }
    }
  ...
  recordTrackedResponse(accountId, claudeResponse, model, error = null) {
    if (!accountId) {
      return;
    }

    OAuthManager.updateAccountQuota(accountId, claudeResponse.headers, {
      orgId: claudeResponse.headers['anthropic-organization-id'] || null
    });

    if (this.isDashboardProbe()) {
      return;
    }
    ...
    OAuthManager.recordProxyResponse(accountId, {
      ...
      model,
      requestId: claudeResponse.headers['request-id'] || null,
      error
    });
  }
    ...
    try {
      ...
      for (const line of lines) {
        ...
        if (data.type === 'message_start' && data.message?.usage) {
          return { usage: data.message.usage };
        }
  ...
  async handleResponse(res, body, presetName = null) {
    try {
      const claudeResponse = await this.executeWithAuthFallback(body, presetName);
      const trackedAccountId = this.currentAuthAccountId;
      const trackedModel = body?.model || null;
      ...
      this.recordTrackedResponse(trackedAccountId, claudeResponse, trackedModel);
      this.streamResponse(res, claudeResponse, trackedAccountId);
  ...
  streamResponse(res, claudeResponse, accountId = null) {
    ...
    if (contentType.includes('text/event-stream')) {
      ...
      if (Logger.getLogLevel() >= 3) {
        ...
        debugStream.on('end', () => {
          if (accountId && latestUsage && !this.isDashboardProbe()) {
            OAuthManager.recordUsage(accountId, latestUsage);
          }
          Logger.debug('\n');
      ...
      claudeResponse.on('end', () => {
        ...
        try {
          ...
          if (accountId && jsonData.usage && !this.isDashboardProbe()) {
            OAuthManager.recordUsage(accountId, jsonData.usage);
          }
  ...
  async probeCurrentQuota(probeBody = null) {
    ...
    const response = await this.executeWithAuthFallback(body, null);
    const responseBody = await new Promise((resolve, reject) => {
      ...
      response.on('end', () => {
        ...
        try {
          ...
          reject(new Error(`Failed to parse probe response: ${error.message}`));
        }
      });
    ...
    if (response.statusCode < 200 || response.statusCode >= 300) {
      const message = responseBody?.error?.message || `Probe failed with status ${response.statusCode}`;
      throw new Error(message);
    }

    const quota = parseQuotaHeaders(response.headers);
    const trackedAccountId = this.currentAuthAccountId;
    if (trackedAccountId && quota) {
      OAuthManager.updateAccountQuota(trackedAccountId, quota, {
        orgId: response.headers['anthropic-organization-id'] || null
      });
    }
    ...
    return {
      ...
      source: this.currentAuthSource || 'oauth',
      account_id: trackedAccountId
    };
  }

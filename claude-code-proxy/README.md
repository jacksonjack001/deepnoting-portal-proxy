...
- Chinese implementation notes: [ARCHITECTURE.zh-CN.md](./ARCHITECTURE.zh-CN.md)
...
- Automatically preserves incoming `anthropic-beta` headers and adds `compact-2026-01-12` when `context_management` is present, so Anthropic compaction requests forwarded through LiteLLM or Claude Code do not fail due to a missing beta header
